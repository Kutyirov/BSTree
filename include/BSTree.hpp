...
class Tree {
  ...
  auto print_direct(std::ostream& out, const BStree::Node<T>* node) -> std::ostream&;  //вывод 3-х обходов
  auto print_symmetric(std::ostream& out, const BStree::Node<T>* node) -> std::ostream&;
  auto print_reverse(std::ostream& out, const BStree::Node<T>* node) -> std::ostream&;
  // auto copy(Node*) -> void;  //копирование дерева
 ...
 public:
  ...
  auto print_units(traversal_order c) const
      -> void;  //вывод узлов вызывает один из 3-х вариантов
